  ...
  return {
    ...
    _resize: function (task, shift, drag) {
      ...
      if (task.end_date - task.start_date < cfg.min_duration) {
        if (drag.left) task.start_date = gantt.calculateEndDate(task.end_date, -minDurationInUnits, cfg.duration_unit, task)
        else task.end_date = gantt.calculateEndDate(task.start_date, minDurationInUnits, cfg.duration_unit, task)
      }
    ...
    on_mouse_down: function (e, src) {
      ...
      if (!drag) {
        ...
        if (drag.mode && drag.mode != config.drag_mode.ignore && config['drag_' + drag.mode]) {
          ...
          if (gantt.isSummaryTask(task) && !(task.auto_scheduling === false) && !config.drag_project && drag.mode != config.drag_mode.progress) {
    ...
    _start_dnd: function (e) {
      ...
      if (
        !cfg['drag_' + drag.mode] ||
        !gantt.callEvent('onBeforeDrag', [id, drag.mode, e]) ||
        !this._fireEvent('before_start', drag.mode, [id, drag.mode, e])
      ) {
        this.clear_drag_state()
      } else {
        delete drag.start_drag
