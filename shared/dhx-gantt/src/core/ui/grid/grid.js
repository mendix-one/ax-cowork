...
Grid.prototype = {
  init: function (container) {
    ...
    this.$grid.innerHTML =
      "<div class='gantt_grid_scale' " + gantt._waiAria.gridScaleRowAttrString() + "></div><div class='gantt_grid_data' " + gridDataAriaAttr + '></div>'
  ...
  _renderGridHeader: function () {
    ...
    for (var i = 0; i < columns.length; i++) {
      ...
      var cell =
        ...
        "'" +
        '>' +
        (!componentLabel ? label : '<div data-component-container></div>') +
        sort +
        '</div>'
      cells.push(cell)
    }
    ...
    for (let i in renderableComponents) {
      gantt.config.external_render.renderElement(
        renderableComponents[i],
        this.$grid_scale.querySelector("[data-column-id='" + i + "'] [data-component-container]"),
      )
    }
