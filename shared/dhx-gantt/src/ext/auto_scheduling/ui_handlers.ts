...
export function attachUIHandlers(gantt: any, linksBuilder: any, loopsFinder: any, connectedGroupsHelper: ConnectedGroupsHelper) {
  const _attachAutoSchedulingHandlers = function () {
    ...
    gantt.attachEvent('onAfterLinkDelete', function (id: LinkID, link: ILink) {
      if (gantt.config.auto_scheduling && !gantt._autoscheduling_in_progress && gantt.isTaskExists(link.target)) {
    ...
    function _preventCircularLink(id: LinkID, link: ILink): boolean {
      ...
      if (gantt.isCircularLink(link)) {
        gantt.callEvent('onCircularLinkError', [link, loopsFinder.getLoopContainingLink(link)])
    ...
    function _datesNotEqual(dateA: Date, dateB: Date, taskA: ITask, taskB: ITask): boolean {
      if (!!dateA !== !!dateB) {
    ...
    function _notEqualTaskDates(task1: ITask, task2: ITask): boolean {
      ...
      if (
        ...
        ((_datesNotEqual(task1.end_date, task2.end_date, task1, task2) || task1.duration !== task2.duration) && task1.type !== gantt.config.types.milestone)
      ) {
        return true
      }
    }
    ...
    let relations
    let movedTask
    gantt.attachEvent('onBeforeTaskDrag', function (id: TaskID, mode: string, task: ITask) {
      if (
    ...
    function updateTaskConstraints(task) {
      ...
      const backwardConstraints = [constraintTypes.SNLT, constraintTypes.FNLT, constraintTypes.MSO, constraintTypes.MFO]
      const forwardConstraints = [constraintTypes.SNET, constraintTypes.FNET, constraintTypes.MSO, constraintTypes.MFO]

      if (gantt.config.schedule_from_end) {
        if (backwardConstraints.indexOf(task.constraint_type) > -1) {
          // keep the constraint type
          if (task.constraint_type == constraintTypes.SNLT || task.constraint_type == constraintTypes.MSO) {
        ...
        if (forwardConstraints.indexOf(task.constraint_type) > -1) {
          // keep the constraint type
          if (task.constraint_type == constraintTypes.SNET || task.constraint_type == constraintTypes.MSO) {
            task.constraint_date = new Date(task.start_date)
          } else {
            task.constraint_date = new Date(task.end_date)
          }
    ...
    function finalizeTaskConstraints(task) {
      // TODO: remove in 7.0
      if (gantt.config.auto_scheduling_compatibility) {
        if (task.constraint_type === gantt.config.constraint_types.SNET || task.constraint_type === gantt.config.constraint_types.FNLT) {
    ...
    function onBeforeLigthboxSaveHandler(taskId: TaskID, task: ITask): boolean {
      if (gantt.config.auto_scheduling && !gantt._autoscheduling_in_progress) {
        ...
        if (_notEqualTaskDates(task, oldTask)) {
          ...
          if (gantt.getConstraintType(task) !== gantt.getConstraintType(oldTask) || +task.constraint_date !== +oldTask.constraint_date) {
            changedConstraint = true
          }
        }
      }
    ...
    const draggedTask = {}
    let timeout
    gantt.attachEvent('onBeforeTaskChanged', function (id: TaskID, mode: string, task: ITask) {
