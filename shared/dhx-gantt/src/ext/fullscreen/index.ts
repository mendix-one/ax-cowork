export default function (gantt: any) {
  ...
  function isExpanded() {
    const element =
      (document as IDocument).fullscreenElement ||
  ...
  function isFullscreenAvailable() {
    try {
      return (
        (document as IDocument).fullscreenEnabled ||
  ...
  function cantFullscreen() {
    if (!gantt.$container) {
      // check is gantt initialized or not
      return true
    }
  ...
  gantt.ext.fullscreen = {
    expand(): void {
      ...
      const requestArguments = element.webkitRequestFullscreen ? [(Element as unknown as IElement).ALLOW_KEYBOARD_INPUT] : []
