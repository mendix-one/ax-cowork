...
export function updateLayoutCells(
  ...
  if (Array.isArray(storedCell) && Array.isArray(newLayoutNode) && Array.isArray(oldLayoutNode)) {
    const length = Math.min(storedCell.length, newLayoutNode.length, oldLayoutNode.length)
