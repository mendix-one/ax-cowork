...
export function diffById<T extends { id: string | number }>(prev: T[], next: T[]) {
  const added: T[] = []
  const removed: T[] = []
  const updated: T[] = []
...
export function findRemovedItems<T extends { id: string | number }>(prev: T[], next: T[]) {
  let removed: T[] = []
...
export function mergeArrays<T extends { id: string | number }>(arr1: T[], arr2: T[]): T[] {
