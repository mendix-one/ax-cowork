...
import { Link, Task } from '@dhx/gantt'
...
export function useData(
  ...
  useEffect(() => {
    ...
    const tasksChanged = !isEqual(prevTasksRef.current, tasks)
    const linksChanged = !isEqual(prevLinksRef.current, links)
    const resourcesChanged = !isEqual(prevResourcesRef.current, resources)
    const resourceAssignmentsChanged = !isEqual(prevResourceAssignmentsRef.current, resourceAssignments)
    const baselinesChanged = !isEqual(prevBaselinesRef.current, baselines)
    ...
    if (tasksChanged || linksChanged) {
      if (resetTasks) gantt.$data.tasksStore.clearAll()
      if (resetLinks) gantt.$data.linksStore.clearAll()
    }
