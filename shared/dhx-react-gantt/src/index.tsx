import React, { useImperativeHandle, useEffect, useRef, useState, forwardRef, ReactElement } from 'react'

import { Gantt, Task, Link, GanttEventCallback, RouterFunction, GanttConfigOptions, GanttPlugins, GroupConfig } from './GanttCore'
...
import { BatchChanges, GanttModals, Marker } from './types/types'
...
const ReactWrapper = forwardRef<any, ReactGanttProps>(function ReactGantt(
  ...
  const { lightboxData, handleSave, handleCancel, handleDelete } = useCustomLightbox(internalInstance, customLightbox)
  ...
  function openEditor(placeholder: HTMLElement, editorType: string, initialValue: any, task: Task) {
    return new Promise<void>((resolve) => {
      ...
      setActiveEditors((editors) => [...editors, { id, placeholder, editorType, initialValue, task, onMount: resolve }])
    })
  }

  function closeEditor(placeholder: HTMLElement) {
    setActiveEditors((editors) => editors.filter((e) => e.placeholder !== placeholder))
  }
  ...
  return (
    <div style={{ height: '100%', minHeight: '200px' }}>
      ...
      <InlineEditorsManager activeEditors={activeEditors} inlineEditors={inlineEditors} closeEditor={closeEditor} ganttInstance={internalInstance.current} />
    </div>
  )
})
