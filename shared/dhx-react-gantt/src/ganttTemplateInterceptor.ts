  ...
  return function ganttTemplateInterceptor(ganttInstance, templateWrapper: (el: React.ReactElement) => React.ReactElement, imperativeRef) {
    ...
    function renderWithoutLayoutEffectWarning(renderFunction: () => string): string {
      ...
      console.error = (...args: any[]) => {
        if (typeof args[0] === 'string' && args[0].includes('flushSync was called from inside a lifecycle method. React cannot flush ')) {
          return
        }
    ...
    ganttInstance.config.external_render = {
      ...
      renderElement: (element, container) => {
        ...
        const html = renderWithoutLayoutEffectWarning(() => renderToString(element))
    ...
    function throttledCleanupBase() {
      ...
      for (const [id, _element] of allPortals) {
    ...
    function renderReactTemplate(result, _args) {
      if (React.isValidElement(result)) {
        ...
        const html = renderWithoutLayoutEffectWarning(() => renderToString(elementForPlaceholder))
