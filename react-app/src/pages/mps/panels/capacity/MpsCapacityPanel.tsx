...
import { MpsCapacityChart } from './MpsCapacityChart'
import { MpsCapacityOee } from './MpsCapacityOee'
import { MpsCapacityConstraints } from './MpsCapacityConstraints'
...
//   • Main content = vertically-stacked sections inside .ax-mps-analysis_scroll so it inherits the
...
const CapacityToolGroupTree = observer(() => {
  ...
  return (
    <div className="ax-mps-capacity_tree">
      <div className="ax-mps-capacity_tree_header">
        <span>Tool groups</span>
      </div>
      <div className="ax-mps-capacity_tree_body">
        {Object.entries(grouped).map(([mod, items]) => (
          <div key={mod} className="ax-mps-capacity_tree_module">
            <div className="ax-mps-capacity_tree_module_label">{mod}</div>
            {items.map((g) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="ax-mps-capacity_tree_card_bar">
                    <div className="ax-mps-capacity_tree_card_bar_fill" style={{ width: `${g.utilization}%`, background: utilColor(g.utilization) }} />
                  </div>
                </button>
              )
            })}
...
const KpiCard = ({ label, value, sub, tone = 'default' }: KpiCardProps) => (
  <div className={`ax-mps-capacity_kpi ax-mps-capacity_kpi__${tone}`}>
    <div className="ax-mps-capacity_kpi_label">{label}</div>
    <div className="ax-mps-capacity_kpi_value">{value}</div>
    {sub && <div className="ax-mps-capacity_kpi_sub">{sub}</div>}
  </div>
)
...
const KpiStrip = observer(({ group }: { group: ToolGroup }) => {
  ...
  return (
    <div className="ax-mps-capacity_kpis">
      <KpiCard
        label="Utilization"
...
// ----- Tools table (PO-style via .ax-mps-capacity_detail scope, see SCSS) ----------------------------------
...
// ----- Panel root ---------------------------------------------------------------------------------
export const MpsCapacityPanel = observer((props: MainPanelControls) => {
  const store = useMpsContext().capacity
  return (
    <AxDisplayPanel type="main" icon="mdiFactory" title="Capacity" tools={<CapacityToolbar />} {...props}>
      <div className="ax-mps-simulation">
        <div className="ax-mps-simulation_body">
          ...
          <div className="ax-mps-simulation_body_content ax-mps-analysis_scroll ax-mps-capacity_detail">
