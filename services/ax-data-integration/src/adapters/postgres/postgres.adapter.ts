...
import { type PgConnectConfig, type PgConnectionFactory, RealPgConnectionFactory } from './pg-connection'
...
@Injectable()
export class PostgresAdapter implements SourceAdapter<PostgresAdapterConfig> {
  readonly type = 'postgres'
  private readonly logger = new Logger(PostgresAdapter.name)

  constructor(private readonly connectionFactory: PgConnectionFactory = new RealPgConnectionFactory()) {}

  async discoverMetadata(config: PostgresAdapterConfig, credentials: unknown): Promise<SourceSchema> {
  ...
  async *stream(config: PostgresAdapterConfig, credentials: unknown, options: StreamOptions = {}): AsyncIterable<AdapterRecord> {
    const connectConfig = this.buildConnectConfig(config, credentials)
    const conn = await this.connectionFactory.connect(connectConfig)
    let rowNumber = 0
    try {
  ...
  private buildConnectConfig(config: PostgresAdapterConfig, credentials: unknown): PgConnectConfig {
    const creds = credentials as Partial<PostgresCredentials> | null | undefined
    if (!creds || typeof creds.username !== 'string' || typeof creds.password !== 'string') {
      throw new Error('PostgresAdapter requires credentials { username, password }')
    }
    return {
      host: config.host,
      port: config.port,
