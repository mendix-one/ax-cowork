...
import { type MssqlConnection, type MssqlConnectionFactory, type MssqlConnectConfig } from './mssql-connection'
import { MssqlAdapter, type MssqlAdapterConfig } from './mssql.adapter'

const baseConfig: MssqlAdapterConfig = {
  host: 'sql.local',
  port: 1433,
  database: 'sales',
...
const validCreds = { username: 'sa', password: 'p' }

function fakeFactory(opts: {
  rows?: Record<string, unknown>[]
  onConnect?: (cfg: MssqlConnectConfig) => void
  closeSpy?: jest.Mock
  streamError?: Error
  ...
  return {
    connect: (cfg) => {
      ...
      const conn: MssqlConnection = {
        // eslint-disable-next-line @typescript-eslint/require-await
        stream: async function* (_query, signal) {
...
describe('MssqlAdapter', () => {
  describe('credentials', () => {
    it('throws when credentials are missing', async () => {
      const adapter = new MssqlAdapter(fakeFactory({}))
    ...
    it('throws when username or password is not a string', async () => {
      const adapter = new MssqlAdapter(fakeFactory({}))
      await expect(adapter.discoverMetadata(baseConfig, { username: 'sa' })).rejects.toThrow(/credentials/)
    })
  })
  ...
  describe('discoverMetadata', () => {
    it('infers field types from sample rows', async () => {
      ...
      const adapter = new MssqlAdapter(fakeFactory({ rows }))
    ...
    it('caps sampling at SAMPLE_LIMIT rows even when the source has more', async () => {
      const rows = Array.from({ length: 50 }, (_, i) => ({ id: i }))
      const adapter = new MssqlAdapter(fakeFactory({ rows }))
      const meta = await adapter.discoverMetadata(baseConfig, validCreds)
      expect(meta.raw).toEqual({ sampleSize: 10 })
    })

    it('closes the connection even when stream throws', async () => {
      const closeSpy = jest.fn()
      const adapter = new MssqlAdapter(fakeFactory({ closeSpy, streamError: new Error('boom') }))
  ...
  describe('stream', () => {
    ...
    it('aborts cleanly when options.signal is already aborted', async () => {
      ...
      controller.abort()
      const adapter = new MssqlAdapter(fakeFactory({ rows: [{ id: 1 }, { id: 2 }] }))
    ...
    it('closes the connection after streaming completes', async () => {
      const closeSpy = jest.fn()
      const adapter = new MssqlAdapter(fakeFactory({ rows: [{ id: 1 }], closeSpy }))
  ...
  describe('connection config', () => {
    it('passes host/port/database + ssl + credentials + trustServerCertificate to the factory', async () => {
      const captured: MssqlConnectConfig[] = []
      const adapter = new MssqlAdapter(
        fakeFactory({
      ...
      await adapter.discoverMetadata(
        { ...baseConfig, options: { ssl: true, connectionTimeoutMs: 5000, queryTimeoutMs: 30000, trustServerCertificate: false } },
        validCreds,
      )
