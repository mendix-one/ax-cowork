...
import { type MysqlConnection, type MysqlConnectionFactory, type MysqlConnectConfig } from './mysql-connection'
import { MysqlAdapter, type MysqlAdapterConfig } from './mysql.adapter'

const baseConfig: MysqlAdapterConfig = {
  host: 'db.local',
  port: 3306,
  database: 'sales',
...
function fakeFactory(opts: {
  ...
  onConnect?: (cfg: MysqlConnectConfig) => void
  closeSpy?: jest.Mock
  streamError?: Error
...
describe('MysqlAdapter', () => {
  describe('credentials', () => {
    it('throws when credentials are missing', async () => {
      const adapter = new MysqlAdapter(fakeFactory({}))
  ...
  describe('discoverMetadata', () => {
    it('infers field types from sample rows including bigint', async () => {
      const rows = [
        { id: 1, big: 9007199254740993n, name: 'Alice', joined_at: new Date('2026-01-01T00:00:00Z'), active: 1 },
        { id: 2, big: 9007199254740994n, name: 'Bob', joined_at: new Date('2026-02-01T00:00:00Z'), active: 0 },
      ]
      const adapter = new MysqlAdapter(fakeFactory({ rows }))
      ...
      expect(byName.big).toBe('integer')
    ...
    it('closes the connection even when stream throws', async () => {
      ...
      const adapter = new MysqlAdapter(fakeFactory({ closeSpy, streamError: new Error('boom') }))
  ...
  describe('stream', () => {
    ...
    it('aborts cleanly when options.signal is already aborted', async () => {
      ...
      controller.abort()
      const adapter = new MysqlAdapter(fakeFactory({ rows: [{ id: 1 }, { id: 2 }] }))
    ...
    it('closes the connection after streaming completes', async () => {
      const closeSpy = jest.fn()
      const adapter = new MysqlAdapter(fakeFactory({ rows: [{ id: 1 }], closeSpy }))
  ...
  describe('connection config', () => {
    it('passes host/port/database + ssl + credentials to the factory', async () => {
      const captured: MysqlConnectConfig[] = []
      const adapter = new MysqlAdapter(
        fakeFactory({
      ...
      await adapter.discoverMetadata({ ...baseConfig, options: { ssl: true, connectionTimeoutMs: 5000, queryTimeoutMs: 30000 } }, validCreds)
