...
describe('Job config manual trigger (e2e)', () => {
  ...
  it('POST /:id/trigger returns 200 with runId + status (graceful failed for unregistered REST adapter)', async () => {
    ...
    const res = await request(app.getHttpServer()).post(`/job-configs/${job.id}/trigger`).set('x-api-key', API_KEY).expect(200)
  ...
  it('POST /:id/trigger returns 404 for a missing job_config id', async () => {
    await request(app.getHttpServer()).post(`/job-configs/${new ObjectId().toHexString()}/trigger`).set('x-api-key', API_KEY).expect(404)
  })

  it('POST /:id/trigger returns 400 for a malformed id', async () => {
    await request(app.getHttpServer()).post('/job-configs/not-an-id/trigger').set('x-api-key', API_KEY).expect(400)
  })
  ...
  it('POST /:id/trigger requires the api key (401 without)', async () => {
