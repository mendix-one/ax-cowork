...
describe('TokenController (e2e)', () => {
  ...
  it('writes separate token rows for different scopes under the same session', async () => {
    ...
    await request(app.getHttpServer()).post('/token').set(API_KEY_HEADER, API_KEY).set('Authorization', `Bearer ${signedInToken}`).send({ scope: TARGET_SCOPE }).expect(201)
    await request(app.getHttpServer()).post('/token').set(API_KEY_HEADER, API_KEY).set('Authorization', `Bearer ${signedInToken}`).send({ scope: 'ANALYTICS' }).expect(201)
