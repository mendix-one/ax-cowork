...
export type SimulationTaskRow = {
  ...
  // attach an alternating `ax-mps-simulation-poband__{a|b}` class so the SCSS can zebra-band by PO group.
  poIndex: number
}
    ...
    const out: SimulationMarker[] = [
      {
        ...
        css: 'ax-mps-simulation-today-marker',
        text: 'TODAY',
    ...
    for (const order of this.filteredOrders) {
      for (const m of order.milestones) {
        out.push({
          ...
          css: `ax-mps-simulation-milestone-marker ax-mps-simulation-milestone-marker__${m.state}`,
