...
import { MilestoneChips, PriorityChip, StateChip, StatusChip } from './production-order-chips'
import { MpsProductionOrderSummary } from './MpsProductionOrderSummary'
import { ganttKeyForBatch, ganttKeyForFamily, ganttKeyForOrder, noteKeyForBatch, noteKeyForFamily, noteKeyForOrder, remarkForOrder, waferStatesForOrder, waferStatesForRow } from './production-order.helpers'
...
const EntityNotesList = observer(({ entityKey, placeholder }: { entityKey: string; placeholder: string }) => {
  ...
  return (
    <div className="ax-po_pn">
      {notes.length === 0 ? (
        <Typography.Text type="secondary" className="text-sm">
          No notes yet — add one below.
        </Typography.Text>
      ) : (
        <div className="ax-po_pn_list">
          {notes.map((n) => (
            <div key={n.id} className="ax-po_pn_item">
              <div className="ax-po_pn_item_meta">
...
const BatchesList = observer(({ poId, familyId }: { poId: string; familyId: string }) => {
  ...
  return (
    <div className="ax-po_fams">
      {family.batches.map((b) => (
        <button key={b.id} type="button" className="ax-po_fams_item" onClick={() => po.selectRow(`batch::${family.id}::${b.id}`)}>
          <div className="ax-po_fams_item_top">
            <span className="ax-po_fams_item_label">{b.name}</span>
            <Tag color="default" style={{ margin: 0 }}>
              {b.waferCount.toLocaleString()} wafers
            </Tag>
            <StateChip state={b.scheduleClass} />
          </div>
          <div className="ax-po_fams_item_meta">
  ...
  return (
    <div className="ax-po_fams">
      {order.schedule.map((f) => {
        ...
        return (
          <button key={f.id} type="button" className="ax-po_fams_item" onClick={() => po.selectRow(`family::${f.id}`)}>
            <div className="ax-po_fams_item_top">
              <span className="ax-po_fams_item_label">{f.label}</span>
              <Tag color="default" style={{ margin: 0 }}>
                {f.tech}
              </Tag>
...
export const MpsProductionOrderInfoPanel = observer(() => {
  ...
  if (row && order) {
    if (row.kind === 'po') {
      const gKey = ganttKeyForOrder(order.id)
      const isIncluded = sim.gantt.checkedKeys.includes(gKey)
      ...
      titleLeft = (
        <span className="ax-po_info_header_title">
          <span className="ax-po_info_header_title_code">{order.id}</span>
          <span className="ax-po_info_header_title_sep">·</span>
          <span className="ax-po_info_header_title_context">{order.customer}</span>
      ...
      titleActions = (
        ...
        >
          <Button
            ...
            onClick={() => sim.gantt.setKeyIncluded(gKey, isExcluded)}
          >
      ...
      const gKey = ganttKeyForFamily(order.id, family.id)
      const isIncluded = sim.gantt.checkedKeys.includes(gKey)
      const isExcluded = !isIncluded
      ...
      titleLeft = (
        <span className="ax-po_info_header_title">
          <span className="ax-po_info_header_title_code">{family.label}</span>
          <span className="ax-po_info_header_title_sep">·</span>
          <span className="ax-po_info_header_title_context">{order.id}</span>
      ...
      const gKey = ganttKeyForBatch(order.id, family!.id, batch.id)
      const isIncluded = sim.gantt.checkedKeys.includes(gKey)
      const isExcluded = !isIncluded
      ...
      titleLeft = (
        <span className="ax-po_info_header_title">
          <span className="ax-po_info_header_title_code">{batch.name}</span>
          <span className="ax-po_info_header_title_sep">·</span>
          <span className="ax-po_info_header_title_context">{family!.label}</span>
          <StateChip state={state} />
        </span>
      )
      titleActions = (
  ...
  return (
    <div className={`ax-po_info ${heightClass}`}>
      <div className="ax-po_info_header">
        <div className="ax-po_info_header_left">{titleLeft}</div>
        <div className="ax-po_info_header_right">
          {titleActions}
          <Button
            size="small"
            type="text"
          ...
          (() => {
            ...
            return (
              <div className="ax-po_info_card">
                {/* ---- Meta Info ------------------------------------------------------------- */}
                <div className="ax-po_info_section">
                  <div className="ax-po_info_section_title">Meta Info</div>
                  <Descriptions
                    size="small"
                    column={2}
                    bordered
                ...
                {/* ---- Production Family ----------------------------------------------------- */}
                <div className="ax-po_info_section">
                  <div className="ax-po_info_section_title">
                ...
                {/* ---- Notes ----------------------------------------------------------------- */}
                <div className="ax-po_info_section">
                  <div className="ax-po_info_section_title">Notes ({po.getNotes(noteKeyForOrder(order.id)).length})</div>
          ...
          (() => {
            ...
            return (
              <div className="ax-po_info_card">
                ...
                {/* ---- Notes ----------------------------------------------------------------- */}
                <div className="ax-po_info_section">
                  <div className="ax-po_info_section_title">Notes ({po.getNotes(noteKey).length})</div>
