...
import { calcStageGroups, formatCycle, formatYield, stageClockHours, stageProportion, type StageGroup } from '../../helpers/process.helpers'
...
// Each section's header is sticky (inherited from .ax-mps-analysis_scroll), so as the planner scrolls,
...
const KpiCard = ({ label, value, sub, tone = 'default' }: KpiCardProps) => (
  <div className={`ax-mps-process_kpi ax-mps-process_kpi__${tone}`}>
    <div className="ax-mps-process_kpi_label">{label}</div>
    <div className="ax-mps-process_kpi_value">{value}</div>
    {sub && <div className="ax-mps-process_kpi_sub">{sub}</div>}
  </div>
)
...
const StageOverviewCard = ({ group }: { group: StageGroup }) => {
  ...
  return (
    <div className="ax-mps-process_overview_card">
      <div className="ax-mps-process_overview_card_top">
        <Tag color={STAGE_TAG_COLOR[group.stage]} style={{ margin: 0 }}>
          {group.stage}
        </Tag>
        <span className="ax-mps-process_overview_card_steps">
          {group.steps.length} step{group.steps.length === 1 ? '' : 's'}
        </span>
      </div>
      <div className="ax-mps-process_overview_card_clock">{formatCycle(clock)}</div>
      <Tooltip
        title={
      ...
      >
        <div className="ax-mps-process_overview_bar" aria-label="time composition">
          <div className="ax-mps-process_overview_bar_seg ax-mps-process_overview_bar_seg__process" style={{ width: `${p.process}%` }} />
          <div className="ax-mps-process_overview_bar_seg ax-mps-process_overview_bar_seg__move" style={{ width: `${p.move}%` }} />
          <div className="ax-mps-process_overview_bar_seg ax-mps-process_overview_bar_seg__wait" style={{ width: `${p.wait}%` }} />
        </div>
      </Tooltip>
    </div>
  )
...
const renderStepTile = (step: ProcessStep, isBottleneck: boolean, onDrill: (toolGroup: string) => void) => {
  ...
  return (
    <div
      className={`ax-mps-process_step ${isBottleneck ? 'is-bottleneck' : ''}`}
      role="button"
      tabIndex={0}
    ...
    >
      <div className="ax-mps-process_step_top">
        <span className="ax-mps-process_step_order">#{step.order}</span>
        <Tag color={STAGE_TAG_COLOR[step.stage]} style={{ margin: 0 }}>
          {step.stage}
        </Tag>
      </div>
      <div className="ax-mps-process_step_name">{step.name}</div>
      <div className="ax-mps-process_step_meta">
        <Tooltip title="Tool group">
          <span className="ax-mps-process_step_chip">
      ...
      <div className="ax-mps-process_step_kpis">
        <div className="ax-mps-process_step_kpi">
          <div className="ax-mps-process_step_kpi_label">Cycle</div>
          <div className="ax-mps-process_step_kpi_value">{formatCycle(step.cycleHours)}</div>
        </div>
        <div className="ax-mps-process_step_kpi">
          <div className="ax-mps-process_step_kpi_label">Yield</div>
          <div className="ax-mps-process_step_kpi_value">{formatYield(step.expectedYield)}</div>
        </div>
      </div>
      <div className="ax-mps-process_step_footer">
...
const renderTransition = (move: number, wait: number, variant: 'inline' | 'large', title: string) => (
  <Tooltip title={title}>
    <div className={`ax-mps-process_transition ax-mps-process_transition__${variant}`}>
      <AxMuiIcon icon="mdiArrowRightThick" size={variant === 'large' ? 22 : 16} className="ax-mps-process_transition_arrow" />
      <div className="ax-mps-process_transition_chips">
        {move > 0 && (
          <span className="ax-mps-process_transition_chip ax-mps-process_transition_chip__move">
            <AxMuiIcon icon="mdiTransitConnectionVariant" size={11} />
            {formatCycle(move)} move
          </span>
        )}
        {wait > 0 && (
          <span className="ax-mps-process_transition_chip ax-mps-process_transition_chip__wait">
  ...
  return (
    <div className="ax-mps-analysis_section">
      ...
      <div className="ax-mps-analysis_section_body">
        ...
        <div className="ax-mps-process_stage_section_row">
          {group.steps.map((step, sIdx) => (
            <div key={step.id} className="ax-mps-process_pipeline_node">
        ...
        <div className="ax-mps-process_stage_section_table">
...
// ===== Main panel ===============================================================================
export const MpsProcessPipeline = observer(() => {
  ...
  return (
    <div className="ax-mps-process_pipeline_wrap">
      {/* ===== 1. Summary KPI strip ============================================================ */}
      <div className="ax-mps-process_kpis">
        <KpiCard
      ...
      <div className="ax-mps-analysis_section">
        ...
        <div className="ax-mps-analysis_section_body">
          <div className="ax-mps-process_overview">
            {stages.map((g, gIdx) => (
              <div key={g.stage} className="ax-mps-process_overview_cell">
          ...
          <div className="ax-mps-process_overview_legend">
            <span className="ax-mps-process_overview_legend_item">
              <span className="ax-mps-process_overview_legend_swatch ax-mps-process_overview_legend_swatch__process" />
              Process
            </span>
            <span className="ax-mps-process_overview_legend_item">
              <span className="ax-mps-process_overview_legend_swatch ax-mps-process_overview_legend_swatch__move" />
              Move
            </span>
            <span className="ax-mps-process_overview_legend_item">
              <span className="ax-mps-process_overview_legend_swatch ax-mps-process_overview_legend_swatch__wait" />
              Wait
            </span>
          </div>
          <div className="ax-mps-process_stage_section_table">
