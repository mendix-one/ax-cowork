...
// 'simulation' because analysis adjustments propagate through the simulation store; PO targets its own store.
// Save-as-new-scenario stays plan-level (sim.openEpsPlanModal) regardless of target.
//
// The "N unsaved" chip is intentionally NOT rendered here — the global counter lives in the app
// top bar (EpsLayoutTop) so the planner sees the aggregate dirty state regardless of which
...
export const EpsScheduleActions = observer(({ target }: { target: ScheduleActionTarget }) => {
  const sim = useEpsContext()
  const store = target === 'simulation' ? sim.simulation : sim.order
  ...
  const saveMenu: MenuProps = {
    ...
    onClick: ({ key }) => {
      ...
      else if (key === 'scenario') sim.openEpsPlanModal()
