...
export const EpsProcessTechList = observer(() => {
  ...
  return (
    <div className="ax-eps-process_tech_list">
      ...
      <div className="ax-eps-process_tech_list_body">
        {summaries.map((s) => {
          const isActive = s.tech === process.selectedTech
          return (
            <button key={s.tech} type="button" className={`ax-eps-process_tech_card ${isActive ? 'is-active' : ''}`} onClick={() => process.selectTech(s.tech)}>
              <div className="ax-eps-process_tech_card_top">
                <div className="ax-eps-process_tech_card_title">{s.tech}</div>
              ...
              <div className="ax-eps-process_tech_card_stats">
                <div>
                  <div className="ax-eps-process_tech_card_stat_label">POs in plan</div>
                  <div className="ax-eps-process_tech_card_stat_value">{s.poCount}</div>
                </div>
                <div>
                  <div className="ax-eps-process_tech_card_stat_label">Wafers</div>
                  <div className="ax-eps-process_tech_card_stat_value">{s.totalWafers.toLocaleString()}</div>
                </div>
                <div>
                  <Tooltip title={`Processing ${formatCycle(s.totalCycleHours)} · Move ${formatCycle(s.totalMoveHours)} · Wait ${formatCycle(s.totalWaitHours)}`}>
                    <div className="ax-eps-process_tech_card_stat_label">End-to-end</div>
                    <div className="ax-eps-process_tech_card_stat_value">{formatCycle(s.totalClockHours)}</div>
                  </Tooltip>
                </div>
                <div>
                  <div className="ax-eps-process_tech_card_stat_label">E2E yield</div>
                  <div className="ax-eps-process_tech_card_stat_value">{formatYield(s.endToEndYield)}</div>
                </div>
              </div>
              {s.bottleneckStep && (
