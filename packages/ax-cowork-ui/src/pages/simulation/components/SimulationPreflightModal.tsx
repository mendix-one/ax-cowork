...
import { runPreflight, summarize, type PreflightFinding } from './preflight.helpers'
...
export type SimulationPreflightModalProps = {
  open: boolean
  onCancel: () => void
  onConfirm: () => void
...
export const SimulationPreflightModal = observer((props: SimulationPreflightModalProps) => {
  ...
  const findings = useMemo(() => runPreflight({ orders: sim.gantt.orders }), [sim.gantt.orders])
  ...
  const drill = (f: PreflightFinding) => {
    ...
    else sim.navigateToProductionOrder(f.drillTarget.id)
    props.onCancel()
  }
