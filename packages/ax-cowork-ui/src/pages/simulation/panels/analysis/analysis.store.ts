...
// from the gantt store (filtered orders) + the shared mock-plan capacity dataset, so the
...
export class AnalysisStore {
  ...
  // (shopFloorBrush), without touching this top-level range.
  ...
  shopFloorBrush: { start: string; end: string } | null = null
  ...
  setShopFloorBrush(range: { start: string; end: string } | null) {
    this.shopFloorBrush = range
  }

  clearShopFloorBrush() {
    this.shopFloorBrush = null
  }
