...
export const ganttKeyForRow = (row: FlatRow): string | null => {
...
// info panel's Exclude button looks up the gantt key from the order object).
export const ganttKeyForOrder = (poId: string) => `po::${poId}`
export const ganttKeyForFamily = (poId: string, familyId: string) => `pf::${poId}::${familyId}`
export const ganttKeyForBatch = (poId: string, familyId: string, batchId: string) => `mb::${poId}::${familyId}::${batchId}`
