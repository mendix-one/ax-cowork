...
export function LoginForm(props: LoginFormProps): ReactElement {
  ...
  // AntD owns typography/spacing tokens, but we pin the primary brand color to
  // axColors.primary so the widget looks identical to the react-app login
  // regardless of the Mendix app's AntD theme.
  return (
    <ConfigProvider theme={{ token: { colorPrimary: '#3F51B5' } }}>
      <div className={`ax-login ${props.className ?? ''}`} style={props.style}>
        ...
        <Form
          ...
          disabled={blocked}
        >
          {props.errorMessage && <Alert type="error" message={props.errorMessage} showIcon style={{ marginBottom: 16 }} />}
          ...
          <Form.Item label={props.passwordLabel} required>
            <Input.Password autoComplete="current-password" value={props.password} onChange={(e) => props.onPasswordChange(e.target.value)} />
          </Form.Item>
