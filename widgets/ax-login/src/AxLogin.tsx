...
export function AxLogin(props: AxLoginContainerProps): ReactElement {
  // Mendix re-renders when status flips, so we don't manage loading state ourselves —
  // we just gate writes/submissions until both attributes are available.
  const isReady = props.accountAttribute.status === 'available' && props.passwordAttribute.status === 'available'
